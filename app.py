...
def index():
    ...
    if request.method == "GET":
        ...
        if len(audio_file) != 0:
            ...
            inputtext = ""
        if len(image_file) != 0:
            inputtext = pytesseract.image_to_string(
                Image.open("./image/" + image_file[0]), lang="vie"
            )
        ...
        with open("data.txt", "r", encoding="utf8") as f1:
            data = f1.read()
        with open("question.txt", "r", encoding="utf8") as f2:
            question = f2.read()
        with open("answer.txt", "r", encoding="utf8") as f3:
            answer = f3.read()
        open("data.txt", "w").close()
        open("question.txt", "w").close()
